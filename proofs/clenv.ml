  ...
  | mv::mvs ->
      ...
      if occur_meta sigma ty then fold metas sigma (mvs@[mv])
        ...
        let naming =
          ...
          | _, Evar_kinds.VarInstance id when Evd.accessible_goal_names() -> Namegen.IntroIdentifier id
...
let mk_goal env evars ?name ?parent hyps concl =
  ...
  | Some name when Evd.accessible_goal_names() -> Evarutil.next_evar_name (Namegen.IntroIdentifier name)
  | _ -> None
  in
  let (evars,evk) =
...
let refiner_gen is_case =
  ...
  | Case ((ci, u, pms, p, iv, c), branches) ->
    ...
    (sigma, accu, ans)
  | Std (metas, r) ->
    std_refine ~metas env sigma ~parent:gl cl r
  in
