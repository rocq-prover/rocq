...
type abstract_args = { transparent : bool; using : Id.t option }
...
let start_subproof gln p =
  ...
  | SubproofAbstract args ->
    ...
    | g :: _ ->
      ...
      focus subproof_cond (Some (args,g,g')) 1 p

let abstract_v, abstract_hook = Hook.make ()
let abstract_v : (abstract_args -> unit Proofview.tactic -> unit Proofview.tactic) Hook.value = abstract_v
...
let end_subproof p0 =
  ...
  | Some (args, g, g') ->
    let tac =
      ...
      tclFOCUS 1 1 begin
        Hook.get abstract_v args begin
