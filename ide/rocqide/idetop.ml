...
let process_goal short sigma g =
  ...
  let name = if Vernacgoal.print_goal_name sigma g then Some (Termops.evar_string env sigma g) else None in
  let ccl =
...
let process_goal_diffs ~short diff_goal_map oldp nsigma ng =
  ...
  let name = if Vernacgoal.print_goal_name nsigma ng then Some (Termops.evar_string env nsigma ng) else None in
