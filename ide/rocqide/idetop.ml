    ...
    if short then [] else
      let hyps =
        List.rev_map process_hyp
          (Ppconstr.compact_named_context sigma (EConstr.named_context_val env))
      in
      hyps
  in
...
let process_goal_diffs ~short diff_goal_map oldp nsigma ng =
  ...
  let name = if Printer.print_goal_name nsigma ng then Some (Termops.evar_string env nsigma ng) else None in
