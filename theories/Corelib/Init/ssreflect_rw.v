...
(**
 ...
 advanced developers; in addition the following are defined:
  ...
  -> The usage pattern for ADT operations is:
     ...
     rw unlock to expose big_foo_expression.
  ...
  - calling rw from an intro pattern, use with parsimony:
    => /#[#1! rules#]#  := rw rules
    => /#[#! rules#]#   := rw !rules
...
Reserved Notation "T (* n *)"
  (at level 200, only printing, format "T  (* n *)").
...
Notation "<hidden n >" := (abstract _ n _) (only printing) : ssr_scope.
Notation "T (* n *)" := (abstract T n abstract_key) (only printing) : ssr_scope.
...
Inductive external_view : Type := tactic_view (_ : Type).
...
Definition get_by vT sT (_ : sT -> vT) := @get vT sT.
...
Definition argumentType T P (_ : forall x : T, P x) := T.
Definition dependentReturnType T P (_ : forall x : T, P x) := P.
Definition returnType aT rT (_ : aT -> rT) := rT.
...
Abbreviation unkeyed x := (let flex := x in flex) (only parsing).
...
(**
 ...
 convertibility during type checking:
  nosimpl t simplifies to t EXCEPT in a definition; more precisely, given
    ...
    a term that beta-iota reduces to bar; hence rw /foo will replace
    foo by bar, and rw -/foo will replace bar by foo.
