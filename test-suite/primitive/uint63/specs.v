...
(** The instances are kept in one list per specification rather than
    in a single flat list: the reduction machines ([cbv], [vm_compute],
    [cbn], ...) recurse along the spine of a list, so a flat list of
    tens of thousands of instances overflows the stack on some
    configurations, whereas one list per specification is at most the
    cube of the length of [tricky_ints]. *)
Definition instantiate_all_ways_nored (ls : list SPEC) : list (list ANNOTATED_BARE_SPEC)
  := ListDef.map instantiate1_all_ways ls.

Definition instantiate_all_ways (ls : list SPEC) : list (list ANNOTATED_BARE_SPEC)
...
(** The results come as one list per specification; report each list,
    and only fail once every list has been reported (unless [error_early]). *)
Ltac2 rec report_results_chunks (error_early : bool) (red : string) (results : constr) : unit
  := lazy_match! results with
     | nil => ()
     | cons ?chunk ?rest
       => let r := Control.case (fun () => report_results_gen error_early red chunk) in
          let rethrow () := match r with
                            | Val _ => ()
                            | Err e => Control.zero e
                            end in
          if error_early
          then (rethrow (); report_results_chunks error_early red rest)
          else (report_results_chunks error_early red rest; rethrow ())
     | _
       => let results' := Std.eval_hnf results in
          if Constr.equal results results'
          then Control.throw (PrimInt_Test_InternalError (fprintf "Results must be a literal list of lists, not %t" results))
          else report_results_chunks error_early red results'
     end.
Ltac2 report_results red results := report_results_chunks false red results.
Ltac2 report_results_fast red results := report_results_chunks true red results.
...
Definition map_fst : list (list ANNOTATED_BARE_SPEC) -> list (list BARE_SPEC)
  := Eval cbv in ListDef.map (ListDef.map (@fst _ _)).
Definition combine_annotations (orig : list (list ANNOTATED_BARE_SPEC)) (result : list (list BARE_SPEC)) : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv in
      ListDef.map (fun '(orig, result) => ListDef.map (fun '((_, anno), v) => (v, anno)) (combine orig result))
        (combine orig result).
...
Definition wrong_specs : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [wrong_spec_list instantiate_all_ways] in instantiate_all_ways wrong_spec_list.

Definition wrong_bare_specs : list (list BARE_SPEC)
  := Eval cbv [map_fst wrong_specs] in map_fst wrong_specs.

Definition wrong_bare_specs_vm : list (list BARE_SPEC)
  := Eval vm_compute in wrong_bare_specs.

(** ** Fuse in the annotations so that we can report errors nicely *)
Definition wrong_results_vm : list (list ANNOTATED_BARE_SPEC)
...
Definition op_specs : list (list ANNOTATED_BARE_SPEC)
  := instantiate_all_ways_nored op_spec_list.
Time Definition op_specs_red : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [instantiate_all_ways op_spec_list] in instantiate_all_ways op_spec_list.
Definition op_bare_specs : list (list BARE_SPEC)
  := map (map fst) op_specs.
Definition op_bare_specs_red : list (list BARE_SPEC)
...
Fixpoint extract_lhs_all (ls : list (list BARE_SPEC)) : list hlist
  := match ls with
     | nil => nil
     | l :: ls => extract_lhs l :: extract_lhs_all ls
     end.
...
Fixpoint merge_lhs_all (ls : list (list BARE_SPEC)) (result : list hlist) : list (list BARE_SPEC)
  := match ls, result with
     | nil, _ | _, nil => nil
     | l :: ls, r :: rs => merge_lhs l r :: merge_lhs_all ls rs
     end.

(** 0. evaluate [op_specs] with [vm_compute] *)
Definition op_bare_specs_vm : list (list BARE_SPEC)
  := Eval vm_compute in op_bare_specs_red.

(** 1. [extract_lhs] of [op_specs] *)
Definition LHS_op : list hlist
  := extract_lhs_all op_bare_specs.
Definition LHS_op_red : list hlist
  := Eval cbv [op_bare_specs_red extract_lhs_all extract_lhs] in extract_lhs_all op_bare_specs_red.
...
Definition LHS_op_native := Eval native_compute in extract_lhs_all op_bare_specs.

(** ** [hnf] *)
(** recursively applies hnf to all elements of the lists *)
...
Ltac2 rec eval_hnf_hlists (c : constr) : constr
  := lazy_match! c with
     | cons ?h ?t =>
         let h := eval_hnf_hlist h in
         let t := eval_hnf_hlists t in
         '(cons $h $t)
     | nil => '(@nil hlist)
     end.
Time Definition LHS_op_hnf := ltac2:(let l := Std.eval_hnf 'LHS_op_red in let x := eval_hnf_hlists l in exact $x).
...
Definition op_bare_specs_native : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_native] in merge_lhs_all op_bare_specs_vm LHS_op_native.
Definition op_bare_specs_hnf : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_hnf] in merge_lhs_all op_bare_specs_vm LHS_op_hnf.
Definition op_bare_specs_cbn : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_cbn] in merge_lhs_all op_bare_specs_vm LHS_op_cbn.
Definition op_bare_specs_simpl : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_simpl] in merge_lhs_all op_bare_specs_vm LHS_op_simpl.
Definition op_bare_specs_cbv : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_cbv] in merge_lhs_all op_bare_specs_vm LHS_op_cbv.
Definition op_bare_specs_lazy : list (list BARE_SPEC)
  := Eval cbv [merge_lhs_all merge_lhs op_bare_specs_vm LHS_op_lazy] in merge_lhs_all op_bare_specs_vm LHS_op_lazy.

(** ** Fuse in the annotations so that we can report errors nicely *)
Time Definition op_results_native : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [combine_annotations op_specs_red op_bare_specs_native] in combine_annotations op_specs_red op_bare_specs_native.
Time Definition op_results_hnf : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [combine_annotations op_specs_red op_bare_specs_hnf] in combine_annotations op_specs_red op_bare_specs_hnf.
Time Definition op_results_cbn : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [combine_annotations op_specs_red op_bare_specs_cbn] in combine_annotations op_specs_red op_bare_specs_cbn.
Time Definition op_results_simpl : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [combine_annotations op_specs_red op_bare_specs_simpl] in combine_annotations op_specs_red op_bare_specs_simpl.
Time Definition op_results_cbv : list (list ANNOTATED_BARE_SPEC)
  := Eval cbv [combine_annotations op_specs_red op_bare_specs_cbv] in combine_annotations op_specs_red op_bare_specs_cbv.
Time Definition op_results_lazy : list (list ANNOTATED_BARE_SPEC)
