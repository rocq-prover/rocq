...
- The dispatch tactical :n:`tac; [foo|bar]` is now written :n:`tac > [foo|bar]`.
  ...
  * **Note on sequencing before dispatch:** Because Ltac2 does not
    automatically delay tactic execution and due to operator precedence, a
    sequence like ``tac1; tac2 > [foo|bar]`` is parsed as ``tac1; (tac2 > [foo|bar])``.
    If ``tac1`` generates multiple goals, the dispatcher will attempt to apply the list
    ``[foo|bar]`` to the subgoals generated by ``tac2`` *independently* for each goal
    produced by ``tac1``. This typically results in an "Incorrect number of goals" error.
    To achieve standard Ltac1 factoring, you must use parentheses to explicitly group
    the sequence: ``(tac1; tac2) > [foo|bar]``.
