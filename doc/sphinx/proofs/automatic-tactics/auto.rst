...
by adding a hint to an unknown database.

  - Databases created with :cmd:`Create HintDb` have the default setting `Transparent`
    for `Constants`, `Projections` and `Variables`.
  - Databases created implicitly have the default `Opaque` setting.

We recommend using :cmd:`Hint Constants`,
...
--------------
   ...
   *(Deprecated since version 8.10:* If no :token:`ident`\s
   ...
   .. cmd:: Hint Resolve {+ {| @qualid | @one_term } } {? @hint_info } {? : {+ @ident } }
      ...
      Note that unlike :tacn:`auto` and :tacn:`eauto`,
