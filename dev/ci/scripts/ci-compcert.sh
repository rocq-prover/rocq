...
( cd "${CI_BUILD_DIR}/compcert"
  [ -e Makefile.config ] || ./configure -ignore-ocaml-version -ignore-coq-version x86_32-linux -install-coqdev -clightgen -use-external-MenhirLib -use-external-Flocq -prefix ${CI_INSTALL_DIR} -coqdevdir ${CI_INSTALL_DIR}/lib/coq/user-contrib/compcert
  make
