...
- [ ] For X.Y+rc1, once opam and Nix packages are ready (and ideally Docker images), send an e-mail on coqdev + coq-club + the Coq announcement category on Discourse (coqdev@inria.fr + coq-club@inria.fr + coq+announcements@discoursemail.com) announcing that the RC is out, so that package maintainers can start preparing package updates and library authors can safely start preparing compatible releases.
