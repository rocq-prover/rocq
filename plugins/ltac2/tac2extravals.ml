...
let interp_constr flags ist c =
  ...
  Tac2api.pf_apply ~catch_exceptions:true begin fun env sigma ->
...
let () =
  ...
  let interp ist c = interp_constr Tac2api.constr_flags ist c in
...
let () =
  let intern = intern_constr_tacexpr in
  let interp ist c = interp_constr Tac2api.open_constr_no_classes_flags ist c in
...
let interp_preterm_var_as_constr ?loc env sigma tycon id =
  ...
  in
  let flags = Tac2api.preterm_flags in
...
type seqrule = SeqRule : 'a TacSyn.seq * ('a -> raw_tacexpr list) -> seqrule

let rec interp_seq_rule = function
  | [] ->
    SeqRule (TacSyn.nil, (fun () -> []))
  | (skipx,synx) :: rest ->
...
let interp_seq_rule toks =
  ...
  let f x = Tac2quote.of_tuple @@ List.rev @@ f x in
