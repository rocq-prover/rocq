...
let interp_constr flags ist c =
  ...
  Tac2core.pf_apply ~catch_exceptions:true begin fun sum env sigma ->
    let (sigma, c) = understand_ltac sum flags env sigma ist WithoutTypeConstraint c in
...
let () =
  let interp sum ?loc ~poly env sigma tycon (ids, tac) =
    ...
    in
    let c, sigma = Proof.refine_by_tactic sum ~name ~poly (GlobEnv.renamed_env env) sigma concl tac in
...
let interp_constr_var_as_constr _sum ?loc env sigma tycon id =
  let ist = Tac2interp.get_env @@ GlobEnv.lfun env in
...
let interp_preterm_var_as_constr sum ?loc env sigma tycon id =
  let open Ltac_pretype in
  let ist = Tac2interp.get_env @@ GlobEnv.lfun env in
  let env = GlobEnv.renamed_env env in
  let c = Id.Map.find id ist.env_ist in
  ...
  in
  let sigma, t, ty = Pretyping.understand_ltac_ty sum flags env sigma vars tycon term in
...
let interp_hyp_var_as_constr _sum ?loc globenv sigma tycon id0 =
...
let () =
  let interp sum ?loc ~poly env sigma tycon (kind,id) =
...
let () = add_syntax_class_full "list0" {
  ...
  interp_synclass = begin fun st -> function
  | subclass, None ->
    let Tac2entries.SyntaxRule (syntax_class, act) = Tac2entries.interp_syntax_class st subclass in
...
let () = add_syntax_class_full "opt" {
  ...
  interp_synclass = begin fun st subclass ->
    let Tac2entries.SyntaxRule (syntax_class, act) = Tac2entries.interp_syntax_class st subclass in
...
let constr_symb st (lev,custom) =
  let custom = Option.map (fun custom -> fst @@ Egramrocq.find_custom_entry_g st custom) custom in
...
let add_constr_classes (name,lname) quote =
  let () =
    let s = name in
    add_syntax_class s (constr_args s) begin fun st -> function (symb,delimiters) ->
      let act e = quote ?delimiters:(Some delimiters) e in
      Tac2entries.SyntaxRule (constr_symb (Procq.FullState.gramstate st) symb, act)
    end
  in
  let () =
    let s = lname in
    add_syntax_class s (constr_delimiters s) begin fun _st -> function delimiters ->
      let act e = quote ?delimiters:(Some delimiters) e in
...
let rec make_seq_rule st = function
| [] ->
  Seqrule (Procq.Rule.stop, CvNil)
| (skip,tok) :: rem ->
  let Tac2entries.SyntaxRule (syntax_class, f) = Tac2entries.interp_syntax_class st tok in
  let syntax_class =
  ...
  in
  let Seqrule (r, c) = make_seq_rule st rem in
  let r = Procq.Rule.next_norec r syntax_class in
  let f = if skip then None else Some f in
  Seqrule (r, CvCns (c, f))

let interp_seq_rule st toks =
  let Seqrule (r, c) = make_seq_rule st (List.rev toks) in
  let syntax_class =
