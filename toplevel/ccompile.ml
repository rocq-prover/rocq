...
let compile opts stm_options injections copts ~echo ~f_in ~f_out =
  ...
  | BuildVo | BuildVok ->
      ...
      let state = Vernac.load_vernac ~beautify ~echo ~check ~state ~source long_f_dot_in in
  ...
  | BuildVos ->
      ...
      let ldir = Stm.get_ldir ~doc:state.doc in
      let source = source ldir long_f_dot_in in
      let state = Vernac.load_vernac ~beautify ~echo ~check:false ~source ~state long_f_dot_in in
