...
let problem_of_qcumul_constraints qcstrs =
  QCumulConstraints.fold (fun (l,d,r) cstrs ->
      match d with
...
let add_univ_constraints uctx cstrs =
  add_univ_constraints_gen QGraph.Static uctx cstrs Quality.ElimConstraints.empty
...
let universe_context_inst decl qvars levels names =
  let leftqs = List.fold_left (fun acc l -> QVar.Set.remove l acc) qvars decl.sort_poly_decl_qualities in
  let leftus = List.fold_left (fun acc l -> Level.Set.remove l acc) levels decl.sort_poly_decl_instance in
  let () =
    let unboundqs = if decl.sort_poly_decl_extensible_qualities then QVar.Set.empty else leftqs in
  ...
  let instq = Array.append
      (Array.map_of_list (fun q -> Quality.QVar q) decl.sort_poly_decl_qualities)
      leftqs
  in
...
let check_template_sort_poly_decl uctx ~template_qvars decl =
  let () =
    match List.filter (fun q -> not @@ QVar.Set.mem q template_qvars) decl.sort_poly_decl_qualities with
    | (_ :: _) as qvars ->
      CErrors.user_err
...
let universe_context_inst_decl decl qvars levels names =
  ...
  let () =
    ...
    then error_unbound_universes unboundqs unboundus names
  in
  let instq = Array.map_of_list (fun q -> Quality.QVar q) decl.sort_poly_decl_qualities in
