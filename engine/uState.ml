...
let set q qv m =
  ...
  | q, QVar qv ->
    ...
    if QSet.mem q m.rigid then None
      ...
      let elims,elim_to_prop,above_prop =
        if is_above_prop m q
        then (QGraph.enforce_eq (QVar qv) (QVar q) m.elims,
              QSet.add qv m.elim_to_prop,
              QSet.add qv (QSet.remove q m.above_prop))
        else (m.elims,m.elim_to_prop,m.above_prop) in
      Some { rigid = m.rigid; qmap = QMap.add q (Some (QVar qv)) m.qmap; above_prop; elims
  ...
  | q, (QConstant qc as qv) ->
    if qc == QSProp && (is_above_prop m q || eliminates_to_prop m q) then None
      ...
      Some { m with rigid = m.rigid; qmap = QMap.add q (Some qv) m.qmap;
                    ...
                    elim_to_prop = QSet.remove q m.elim_to_prop }
...
let set_above_prop q m =
  ...
  if QSet.mem q m.rigid then None
    ...
    Some { rigid = m.rigid; qmap = m.qmap; above_prop = QSet.add q m.above_prop;
...
let union ~fail s1 s2 =
  ...
  let s = { rigid = QSet.union s1.rigid s2.rigid; qmap; above_prop;
            elims; initial_elims; elim_to_prop } in
