...
let check_mono_univ_decl uctx decl =
  ...
  let () =
    ...
    then check_universe_context_set ~prefix levels uctx.names
  in
  (* FIXME: check sort constraints from decl.univdecl_elim_constraints *)
  if decl.univdecl_extensible_constraints then PConstraints.ContextSet.univ_context_set uctx.local
  else begin
    check_implication uctx
      (univ_decl_csts decl) csts;
...
let check_univ_decl ~poly uctx decl =
  ...
  let entry =
    ...
    else Monomorphic_entry (PConstraints.ContextSet.of_univ_context_set (check_mono_univ_decl uctx decl)) (* XXX *)
  in
  entry, binders
