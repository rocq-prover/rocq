...
type sort_context_set = (Quality.QVar.Set.t * Univ.Level.Set.t) * PConstraints.t

type 'a in_sort_context_set = 'a * sort_context_set

let empty_sort_context = (Quality.QVar.Set.empty, Level.Set.empty), PConstraints.empty

let is_empty_sort_context ((qs,us),csts) =
  Quality.QVar.Set.is_empty qs && Level.Set.is_empty us && PConstraints.is_empty csts

let sort_context_union ((qs,us),csts) ((qs',us'),csts') =
  ((Quality.QVar.Set.union qs qs', Level.Set.union us us'), PConstraints.union csts csts')

let diff_sort_context ((qs,us),csts) ((qs',us'),csts') =
  (Quality.QVar.Set.diff qs qs', Level.Set.diff us us'), PConstraints.diff csts csts'
...
let fresh_sort_in_quality =
  ...
  | Qual (QConstant QType | QVar _ (* Treat as Type *)) ->
     ...
     sort_of_univ (Univ.Universe.make u), ((Quality.QVar.Set.empty,Level.Set.singleton u), PConstraints.empty)
