...
let _ = CErrors.register_handler begin function
  | Not_found -> Some (CErrors.user_err (Pp.str "No such goal."))
  | _ -> None
end

(** Internal representation of qualified evar names.

    Example: "x.y.z" is represented as [{ basename: "z"; path: ["y"; "z"] }]
    Note the use of [Seq.t] for representing qualified names without
    computing the full path, which may be costly.
