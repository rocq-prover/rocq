...
let accessible_goal_names = Evarnames.accessible_goal_names
...
let add_with_name (type a) ?name ?parent ~typeclass_candidate d e (i : a evar_info) = match i.evar_body with
| Evar_empty ->
  let evar_names =
    match name with
    | Some (basename, fresh) ->
...
let new_pure_evar ?(src=default_source) ?(filter = Filter.identity) ~relevance
  ...
  ?name ?parent ?(typeclass_candidate = false) sign evd typ =
  let evi = {
    evar_hyps = sign;
  ...
  let evd = add_with_name evd ?name ?parent ~typeclass_candidate newevk evi in
