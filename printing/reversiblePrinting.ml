...
(* Successively more explicit printing flags: coercions, implicit
   ...
   arguments; the next rung additionally prints unnamed universe levels
   and sort quality variables anonymously, because an unnamed level or
   quality variable otherwise prints as a raw name that cannot be
   reparsed, whereas [_] reparses to a fresh universe that unification
   can match. Unsetting notations is tried after sorts and before
   universes. At each universes rung, anonymous universes are tried
   first and then without them, in case anonymizing a universe is itself
   what breaks reparsing. Configurations are ordered by explicitness, not
   included in each other: notations are not kept off once universes are
   shown, and [anonymous_univs] is turned back off whenever it did not
   help. Raw printing stays faithful, with [anonymous_univs] off. Only
...
let ladder flags =
  ...
  let e2a = { e2 with Extern.anonymous_univs = true } in
  let e3 = { e2 with Extern.notations = false } in
  let e3a = { e3 with Extern.anonymous_univs = true } in
