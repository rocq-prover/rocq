  ...
  | LocalAssum (na, t) as decl :: ctx ->
    let sigma, decl, s =
      ...
      | Some _ ->
        ...
        | Sorts.Type u | Sorts.GQSort (_, u) | Sorts.VQSort (_, u) -> TemplateUniv u
        in
...
let check_binder_relevance env sigma s decl =
  let preunify = match ESorts.kind sigma s, ERelevance.kind sigma (get_relevance decl) with
    | (Prop | Set | Type _ | GQSort _), Relevant -> Trivial
    | (Prop | Set | Type _ | GQSort _), Irrelevant -> Impossible
    | SProp, Irrelevant -> Trivial
    | SProp, Relevant -> Impossible
    | VQSort (_,l), RelevanceVar q' -> DummySort (ESorts.make (Sorts.vqsort q' l))
    | (SProp | Prop | Set), RelevanceVar q ->
      DummySort (ESorts.make (Sorts.vqsort q Univ.Universe.type0))
    | (Type l | GQSort (_, l)), RelevanceVar q -> DummySort (ESorts.make (Sorts.vqsort q l))
    | VQSort (_,l), Relevant ->
       begin
    ...
    | VQSort _, Irrelevant -> DummySort ESorts.sprop
  in
