  ...
  let rec unirec_rec (curenv,nb as curenvnb) pb opt (substn : subst0) ?(nargs=0) curm curn =
    ...
    in
      match (EConstr.kind sigma cM, EConstr.kind sigma cN) with
        ...
        | Case (ci1, u1, pms1, (p1,_), iv1, c1, cl1), Case (ci2, u2, pms2, (p2,_), iv2, c2, cl2) ->
            (try
             ...
             let cl1ctx, p1ctx = EConstr.case_expand_contexts env (ci1.ci_ind, u1) pms1 (fst p1) cl1 in
             ...
             in
             let substn = unif opt' substn p1ctx p1 p2 in
             let substn = unirec_rec curenvnb CONV opt' substn c1 c2 in
             Array.fold_left3 (unif {opt with at_top = true}) substn cl1ctx cl1 cl2
             with ex when precatchable_exception ex ->
