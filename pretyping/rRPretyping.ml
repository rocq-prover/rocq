...
let push_constraints env evd sigma =
  ...
  let push_qconstraints sigma =
    let qgraph, qcstrs = Rewrite_rules_ops.QState.merge_qconstraints sigma.qcstrs sigma.qgraph in
    { sigma with qgraph; qcstrs }, Sorts.QCumulConstraints.(cardinal qcstrs <> cardinal sigma.qcstrs)
  in
...
let eval_pretyper_pattern env evd c =
  ...
  let (qgraph, qabove_prop) = Rewrite_rules_ops.QState.to_pair sigma.qgraph in
  let qcstrs = Rewrite_rules_ops.QState.to_qconstraints sigma.qgraph in
