...
and evar_eqappr_x ?(rhs_is_already_stuck = false) flags env evd pbty
  ...
  match (flex_kind_of_term flags env evd term1 sk1,
    ...
    | MaybeFlexible (v1', sk1' as vsk1'), MaybeFlexible (v2', sk2' as vsk2') -> begin
        ...
        | _, _ ->
        let exception Is_proj of ((Names.Constant.t * EConstr.EInstance.t) * (EConstr.constr list option * EConstr.constr * Reductionops.Stack.t)) option in
        (try
          if lastUnfolded = Some true then raise (Is_proj None) else
          let (_, (_, c1, _)) as p1 = try decompose_proj env evd appr1 with Not_found -> raise (Is_proj None) in
          let c1 = whd_all env evd c1 in
          let () = match kind evd c1 with
            | App (h, _) when isConstruct evd h -> ()
            | Construct _ -> ()
            | _ when is_ground_term evd c1 -> raise (Is_proj None)
            | _ -> raise (Is_proj (Some p1)) in
          evar_eqappr_x flags env evd pbty hds lastUnfolded
            (whd_betaiota_deltazeta_for_iota_state
               flags.open_ts env evd vsk1')
            appr2
        with Is_proj p1 -> try
          if lastUnfolded = Some false then raise (Is_proj None) else
          let (_, (_, c2, _)) as p2 = try decompose_proj env evd appr2 with Not_found -> raise (Is_proj None) in
          let c2 = whd_all env evd c2 in
          let () = match kind evd c2 with
            | App (h, _) when isConstruct evd h -> ()
            | Construct _ -> ()
            | _ when is_ground_term evd c2 -> raise (Is_proj None)
            | _ -> raise (Is_proj (Some p2)) in
          evar_eqappr_x flags env evd pbty hds lastUnfolded appr1
            (whd_betaiota_deltazeta_for_iota_state
        ...
        and f2 i =
          if not flags.with_cs then UnifFailure (i,NoCanonicalStructure) else
          let cs1 i =
            match p1 with | None -> UnifFailure (i,NoCanonicalStructure) | Some p1 ->
            get_cs flags env i p1 appr1 (snd hds) in
          let cs2 i =
            match p2 with | None -> UnifFailure (i,NoCanonicalStructure) | Some p2 ->
            get_cs flags env i p2 appr2 (fst hds) in
          ise_try i [cs1; cs2]
        and f3 i =
    ...
    | MaybeFlexible vsk1', Rigid ->
        ...
        let f3 i =
           ...
           else match p1 with | None -> UnifFailure (i,NoCanonicalStructure) | Some p1 ->
           get_cs flags env i p1 appr1 (snd hds)
        and f4 i =
          evar_eqappr_x flags env i pbty hds (Some false)
            (whd_betaiota_deltazeta_for_iota_state
               flags.open_ts env i vsk1')
    ...
    | Rigid, MaybeFlexible vsk2' ->
        ...
        let f3 i =
           if not flags.with_cs then UnifFailure (i,NoCanonicalStructure)
           else match p2 with | None -> UnifFailure (i,NoCanonicalStructure) | Some p2 ->
           get_cs flags env i p2 appr2 (fst hds)
        and f4 i =
          evar_eqappr_x flags env i pbty hds (Some true) appr1
            (whd_betaiota_deltazeta_for_iota_state
               flags.open_ts env i vsk2')
