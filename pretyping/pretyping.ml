...
let search_guard ?loc ?evars env sigma {possibly_cofix; possible_fix_indices} fixdefs =
  ...
  if one_fix_possibility && not possibly_cofix then
    ...
    try let sigma = check_fix_with_elims ?evars env sigma fix in sigma, Some indexes
    with reraise ->
       ...
       let () = List.iter
         (fun l ->
            ...
            (* spiwack: We search for a unspecified structural
               ...
               not specified. *)
            try
              let _ = check_fix_with_elims ?evars env sigma fix in raise (Found (Some indexes))
            with TypeError _ -> ())
          combinations in
       let () =
