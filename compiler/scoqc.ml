...
let parse ~st pa =
  ...
  Procq.Entry.parse Pvernac.(main_entry mode) pa
...
let compile_file ~args ~injections ~root_st ~out_file in_file =
  ...
  let pa = Procq.Parsable.make ~loc (Gramlib.Stream.of_channel f_in) in
