...
let map_red_expr_gen f g h i = function
  ...
  | CbvVmNoStuck occs_o -> CbvVmNoStuck (Option.map (map_occs (Util.map_union g h)) occs_o)
  | CbvVmWhnf occs_o -> CbvVmWhnf (Option.map (map_occs (Util.map_union g h)) occs_o)
