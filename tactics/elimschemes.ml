  ...
  @@ Libobject.superglobal_object "prop_but_default_dependent_elim"
       ~cache:(fun i ->
         prop_but_default_dependent_elim :=
           Indset_env.add i !prop_but_default_dependent_elim)
...
let pseudo_sort_quality_for_elim ind mip =
  ...
  if Sorts.is_prop s && is_prop_but_default_dependent_elim ind then
    Sorts.Quality.qtype
...
let default_case_analysis_dependence env ind =
  ...
  && ((not (Sorts.is_prop mip.mind_sort))
     || is_prop_but_default_dependent_elim ind)
...
let rect_dep =
  declare_individual_scheme_object "rect_dep" (fun env _ x ->
      build_induction_scheme_in_type env true QualityOrSet.qtype x)

let rec_dep =
  declare_individual_scheme_object "rec_dep" (fun env _ x ->
      build_induction_scheme_in_type env true QualityOrSet.set x)

let ind_dep =
  declare_individual_scheme_object "ind_dep" (fun env _ x ->
      build_induction_scheme_in_type env true QualityOrSet.prop x)

let sind_dep =
  declare_individual_scheme_object "sind_dep" (fun env _ x ->
...
let rect_nodep =
  declare_individual_scheme_object "rect_nodep" (fun env _ x ->
      build_induction_scheme_in_type env false QualityOrSet.qtype x)

let rec_nodep =
  declare_individual_scheme_object "rec_nodep" (fun env _ x ->
      build_induction_scheme_in_type env false QualityOrSet.set x)

let ind_nodep =
  declare_individual_scheme_object "ind_nodep" (fun env _ x ->
      build_induction_scheme_in_type env false QualityOrSet.prop x)

let sind_nodep =
  declare_individual_scheme_object "sind_nodep" (fun env _ x ->
  ...
  | Qual q -> begin
      ...
      | QGlobal _ ->
          CErrors.user_err
            Pp.(
              str
                "Cannot automatically lookup elimination scheme for global \
                 sort.")
    end
...
let elimination_suffix =
  ...
  | Qual (QGlobal _) ->
      CErrors.user_err
        Pp.(
          str "Cannot automatically lookup elimination scheme for global sort.")
    ...
    with Not_found ->
      CErrors.user_err
        Pp.(
          ...
          ++ Id.print id
          ++ strbrk ", the elimination of the inductive definition "
          ++ Nametab.pr_global_env Id.Set.empty (GlobRef.IndRef ind_sp)
          ++ strbrk " on sort "
          ++ UnivGen.QualityOrSet.pr
               (UnivNames.quality_printer UnivNames.empty_binders)
               s
          ++ strbrk " is probably not allowed.")

let deprecated_lookup_by_name =
  CWarnings.create ~name:"deprecated-lookup-elim-by-name"
    ~category:Deprecation.Version.v9_2
    Pp.(
      fun (env, ind, to_kind, r) ->
        let pp_scheme () s = str (scheme_kind_name s) in
        fmt
          "Found unregistered eliminator %t for %t by name.@ Use \"Register \
           Scheme\" with it instead@ (\"as %a\" if dependent or \"as %a\" if \
           non dependent)."
          (fun () -> Termops.pr_global_env env r)
          (fun () -> Termops.pr_global_env env (IndRef ind))
          pp_scheme
          (elim_scheme ~dep:true ~to_kind)
          pp_scheme
          (elim_scheme ~dep:false ~to_kind))
...
let lookup_eliminator env ind s =
  ...
  let schemes =
    List.map
      (fun dep -> elim_scheme ~dep ~to_kind:s)
...
let case_dep =
  declare_individual_scheme_object "case_dep" (fun env _ x ->
      build_case_analysis_scheme_in_type env true QualityOrSet.qtype x)

let case_nodep =
  declare_individual_scheme_object "case_nodep" (fun env _ x ->
      build_case_analysis_scheme_in_type env false QualityOrSet.qtype x)

let case_poly_dep q =
  declare_individual_scheme_object "case_poly_dep" (fun env _ x ->
      build_case_analysis_scheme_in_type env true (QualityOrSet.Qual q) x)

let case_poly_nodep q =
  declare_individual_scheme_object "case_poly_nodep" (fun env _ x ->
      build_case_analysis_scheme_in_type env false (QualityOrSet.Qual q) x)

let casep_dep =
  declare_individual_scheme_object "casep_dep" (fun env _ x ->
      build_case_analysis_scheme_in_type env true QualityOrSet.prop x)

let casep_nodep =
  declare_individual_scheme_object "casep_nodep" (fun env _ x ->
      build_case_analysis_scheme_in_type env false QualityOrSet.prop x)

let scase_dep =
  declare_individual_scheme_object "scase_dep" (fun env _ x ->
      build_case_analysis_scheme_in_type env true QualityOrSet.sprop x)

let scase_nodep =
  declare_individual_scheme_object "scase_nodep" (fun env _ x ->
      build_case_analysis_scheme_in_type env false QualityOrSet.sprop x)
