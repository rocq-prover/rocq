...
let guess_elim_shape env sigma isrec s hyp0 =
  ...
  let sigma, elim, nparams =
    if is_elim then
      let sigma, elim = find_ind_eliminator env sigma mind s in
      let elim = mkConstU elim in
  ...
  sigma, elim, (mind, u), (hd, params, indices)
...
| None ->
  let sigma, elim, ind, typ = guess_elim_shape env sigma isrec sort hyp0 in
  (* We drop the scheme and elimc/elimt waiting to know if it is dependent, this
    needs no update to sigma at this point. *)
  let elim = match elim with
    | Some elim -> ElimOver (hyp0, ind, elim)
    | None -> CaseOver (hyp0, ind)
  in
  sigma, typ, elim
...
let apply_induction_in_context with_evars inhyps elim indvars names =
  Proofview.Goal.enter begin fun gl ->
    ...
    | ElimUsing (hyp0, _) | ElimOver (hyp0, _, _) | CaseOver (hyp0, _) -> Some hyp0
