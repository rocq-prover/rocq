...
let top_tree : type s tr a. s ty_entry -> (s, tr, a) ty_tree -> (s, tr, a) ty_tree parser_v =
  ...
  | Node (NoRec3, {node = s; brother = bro; son = son}) ->
    let+ s' = top_symb entry s in
    Node (NoRec3, {node = s'; brother = bro; son = son})
  | LocAct (_, _) | DeadEnd -> Error empty_error
...
let make_continue_parser_of_entry entry desc =
  match desc with
  | Dlevels [] -> (fun _ _ _ _ _ (_ : _ LStream.t) -> Error empty_error)
  | Dlevels elev ->
  ...
  | Dparser p -> fun gstate levfrom levn bp a (strm__ : _ LStream.t) -> Error empty_error
