...
open Univ
open Sorts
type kind = QCumulConstraint.kind = Eq | Leq
open Names
open Constr
open Declarations
open Environ
...
module QState = struct
  ...
  let unify_quality cv_pb q1 q2 qmap =
    ...
    | Eq, QVar qv, q
    | Eq, q, QVar qv -> set qv q qmap, false
  ...
  let merge_qconstraints qcstrs qmap =
    let pre, post = List.partition
      (function (_, QCumulConstraint.Eq, _) | (QConstant (QSProp | QType), _, _) | (_, _, QConstant (QProp | QSProp)) -> true
        | _ -> false) (QCumulConstraints.elements qcstrs)
    in
    ...
    qmap, List.fold_left (fun s c -> QCumulConstraints.add c s) QCumulConstraints.empty (pre' @ post')

  let add_quality qv (qmap : t) : t = { qmap with qmap = QMap.add qv None qmap.qmap }


  let to_qconstraints qmap =
    let qcstrs = QCumulConstraints.empty in
    let qcstrs = QMap.fold (fun q qo' qcstrs -> match qo' with Some q' -> QCumulConstraints.add (QVar q, Eq, q') qcstrs | None -> qcstrs) qmap.qmap qcstrs in
    let qcstrs = QSet.fold (fun q qcstrs -> QCumulConstraints.add (QConstant QProp, Leq, QVar q) qcstrs) qmap.above_prop qcstrs in
    qcstrs
...
let empty = {
  ...
  equalities = [];
  qcstrs = QCumulConstraints.empty;
  ucstrs = [];
  qgraph = QState.empty;
...
let add_quality q ~bound evd =
  ...
  { evd with qualities = QVar.Map.add q bound evd.qualities; qgraph = QState.add_quality q evd.qgraph }
...
let enforce_cmp_quality cv_pb q1 q2 evd =
  let cv_pb = match cv_pb with CONV -> QCumulConstraint.Eq | CUMUL -> Leq in
  let qgraph, b = QState.unify_quality cv_pb q1 q2 evd.qgraph in
  let qcstrs = if b then QCumulConstraints.add (q1, cv_pb, q2) evd.qcstrs else evd.qcstrs in
...
let compare_instances u1 u2 evd =
  let (qcstrs, ulcstrs) = UVars.enforce_eq_instances u1 u2 (QCumulConstraints.empty, evd.ulcstrs) in
  let qgraph, qcstrs = QState.merge_qconstraints qcstrs evd.qgraph in
  { evd with qgraph; ulcstrs; qcstrs = QCumulConstraints.union qcstrs evd.qcstrs }

let compare_cumul_instances cv_pb variance u u' evd =
  let qucstrs = QCumulConstraints.empty, evd.ulcstrs in
...
let rec fterm_relevance infos evd lft c =
  ...
  QState.nf_relevance evd.qgraph @@
...
let rec _unify cv_pb infos ?(elim=Elim None) lft1 t1 lft2 t2 evd =
  ...
  | RelevanceVar q, RelevanceVar q' when QState.check_eq_qvar q q' evd.qgraph ->
...
let push_equality env evd defs (rels, q, t1, cv_pb, t2) =
  match QState.relevance q evd.qgraph with
...
let push_constraints env evd defs =
  ...
  let push_qconstraints evd =
    let qgraph, qcstrs = QState.merge_qconstraints evd.qcstrs evd.qgraph in
    { evd with qgraph; qcstrs }, QCumulConstraints.cardinal qcstrs <> QCumulConstraints.cardinal evd.qcstrs
  in
...
let push_uconstraint qgraph (q, u1, cv_pb, u2) ucstrs =
  let open Sorts.Quality in
  match QState.nf_quality qgraph q with
...
let nf_evar evars qgraph defs c =
  ...
  in
  let c = nf_evar c in
  let usubst = QVar.Map.mapi (fun qv q -> Option.default (Sorts.Quality.QVar qv) q) qgraph.QState.qmap, Level.Map.empty in
  let subst_rel = Sorts.relevance_subst_rel_fn (fun qv -> QState.relevance qv qgraph) in
...
let check_inferred_constraints env evd (Info info) =
  ...
  let evar_map = nf_evar_map info.evars (QState.of_pair info.qualities (info.qgraph, info.qabove_prop)) info.evar_defs evd.evar_map in
  ...
  in
  let qcstrs = QState.to_qconstraints @@ QState.of_pair info.qualities (info.qgraph, info.qabove_prop) in
  if not @@ QCumulConstraints.for_all (fun qcstr -> QState.check_constraint qcstr evd.qgraph) qcstrs then
...
let test_qvar env nvarqs rel_vars ~rel q =
  ...
  | None ->
      match QGraph.check_declared_qualities (Environ.qualities env) (Sorts.Quality.Set.singleton (QVar q)) with
      | Ok () -> ()
      | Error l ->
        let q = Sorts.Quality.Set.choose l in (* Subsingleton of size 1 *)
        match q with Sorts.Quality.QConstant _ -> assert false | QVar q ->
