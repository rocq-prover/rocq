...
let compute_elim_squash ?(is_real_arg=false) env u info =
  ...
  let info = if not is_real_arg then info
      ...
      | NoRelevantArg -> match u with
        ...
        | Prop | Set | Type _ | GQSort _ -> { info with record_arg_info = HasRelevantArg }
  in
  ...
  if not @@ UGraph.check_leq (universes env) (Sorts.univ_of_sort u) (Sorts.univ_of_sort indu) then
    ...
    | VQSort (q, _), (SProp | Prop) when Environ.Internal.is_above_prop env q -> info
      ...
      then Result.Error MustNotBeSquashed
        ...
        | None -> (* Otherwise, we allow primitive projections but check if it has eta *)
            ...
            | NoRelevantArg ->
              ...
              | GQSort _ | Set | Type _ | Prop -> Result.Ok NoEta (* relevant sorts don't have eta *)
              | VQSort _ ->  Result.Ok MaybeEta (* For sort variables it depends on the instantiation *)
    )
    (Result.Ok NoEta)
    data
