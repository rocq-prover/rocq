...
type t =
  ...
  | Set
  | Type of Universe.t
  | GQSort of QGlobal.t * Universe.t
  | VQSort of QVar.t * Universe.t

let sprop = SProp
let prop = Prop
let set = Set
let type1 = Type Universe.type1
let gqsort q u = GQSort (q, u)
let vqsort q u = VQSort (q, u)
...
let univ_of_sort s =
  ...
  | Type u | GQSort (_, u) | VQSort (_, u) -> u

let make q u =
  let open Quality in
  match q with
  | QVar q -> vqsort q u
  | QGlobal q -> gqsort q u
...
let super = function
  ...
  | Type u | GQSort (_, u) | VQSort (_, u) -> Type (Universe.super u)
...
| Type u | GQSort (_, u) | VQSort (_, u) -> Universe.levels u
    ...
    struct
      ...
      let hashcons = function
        ...
        | GQSort (q, u) as c ->
          ...
          let hu, u' = Universe.hcons u in
          combinesmall 3 (combine hu hq), if u' == u && q' == q then c else GQSort (q', u')
        | VQSort (q, u) as c ->
          let hq, q' = QVar.hcons q in
          let hu, u' = Universe.hcons u in
          combinesmall 4 (combine hu hq), if u' == u && q' == q then c else VQSort (q', u')
        | SProp | Prop | Set as s -> hash s, s

      let eq s1 s2 = match (s1,s2) with
        | SProp, SProp | Prop, Prop | Set, Set -> true
        | (Type u1, Type u2) -> u1 == u2
        | GQSort (q1, u1), GQSort (q2, u2) -> q1 == q2 && u1 == u2
        | VQSort (q1, u1), VQSort (q2, u2) -> q1 == q2 && u1 == u2
...
let relevance_of_sort = function
  | SProp -> Irrelevant
  | Prop | Set | Type _ | GQSort _ -> Relevant
  | VQSort (q, _) -> RelevanceVar q
...
let debug_print = function
  ...
  | GQSort (q, u) -> Pp.(str "QSort(" ++ QGlobal.raw_pr q ++ str ","
                        ++ spc() ++ Univ.Universe.raw_pr u ++ str ")")
  | VQSort (q, u) -> Pp.(str "VQSort(" ++ QVar.raw_pr q ++ str ","
                        ++ spc() ++ Univ.Universe.raw_pr u ++ str ")")
...
let extract_sort_level = function
  | Type u | GQSort (_, u) | VQSort (_, u) -> extract_level u
...
let pattern_match ps s qusubst =
  ...
  | PSGlobal (qg, uio), GQSort (qg', u) -> if QGlobal.equal qg qg' then Some (Partial_subst.maybe_add_univ uio (extract_level u) qusubst) else None
