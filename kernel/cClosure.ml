...
let red_transparent =
  let id_ts =
    ...
    in
    TransparentState.{tr_var = Names.Id.Pred.empty; tr_cst; tr_prj = Names.PRpred.empty}
  in
...
module Debug = struct
  ...
  let rec pp_constr : constr pp = fun ff c ->
    ...
    | String s ->
        out "String(%s)" (Pstring.to_string s)
    | Array(_,t,def,ty) ->
  ...
  let rec pp_fterm : fterm pp = fun ff term ->
    ...
    | FConstruct ((((sp,i),j),_), args) ->
        out "FConstruct(%s,%i,%i,%a)" (Names.MutInd.to_string sp) i j (pp_array pp) args
    | FApp (h, args) ->
    ...
    | FString s ->
        out "FString(%s)" (Pstring.to_string s)
    | FArray (_, _, _) ->
        out "FArray(_, _, _)"
    | FLIFT (i, c) ->
...
let rec to_constr ~(info:clos_infos) ~(tab:clos_tab) ((lfts, usubst) as ulfts) v =
  ...
  match v.term with
    ...
    | FConstruct (op,args) ->
      let c = subst_us (mkConstructU op) in
      if Array.is_empty args then c
      else mkApp (c, Array.Fun1.map to_constr ulfts args)
...
let subst_context info tab env ctx =
  if Esubst.is_subs_id (fst env) then
    ...
    let subs = comp_subs ~info ~tab (el_id, UVars.Instance.empty) env in
...
let unfold_projection ~mode info p r =
  ...
  then Some (Zproj (Projection.repr p, r, mode))
  else None
...
let match_symbol red info tab ~pat_state fl (u, b, r) stk =
  let unfold_fix = b && red_set normal_whnf info.i_flags fFIX in
  ...
  | FEta(n,h,args,k,e) ->
    (match get_eta_args mode n h args k e stk with
     | Inl e', s ->
         ...
         knit_nonlexical_app ~mode:mode info tab ~pat_state e' h args s
...
and knit_nonlexical_app ~mode info tab ~pat_state (e : usubs) h args stk =
