...
(* cases where knh stops *)
    | (FFlex _|FLetIn _|FLetInF _|FEvar _|FCaseInvert _|FIrrelevant|
...
val match_symbol : ('a, 'a patstate) reduction -> clos_infos -> Table.t ->
  pat_state:(fconstr, stack, frel_context, 'a) depth -> table_key -> UVars.Instance.t * bool * machine_rewrite_rule list -> stack -> 'a
...
and match_head : 'a. ('a, 'a patstate) reduction -> _ -> _ -> pat_state:(fconstr, stack, _, 'a) depth -> _ -> _ -> _ -> _ -> _ -> _ -> 'a =
  ...
  | FLambdaF (n, ty, body) ->
    ...
    let pty, pbody, elims, states = extract_or_kill4 (fun ((ptys, pbod, elims), psubst) ->
        match Array.length ptys with
        | 0 -> assert false
        | 1 -> Some (ptys.(0), ERigid pbod, elims, psubst)
        | n ->
          let rem = Array.sub ptys 1 (n-1) in
          Some (ptys.(0), ERigid (PHLambda (rem, pbod), []), elims, psubst)
      ) tysbodyelims states
    in
    let decl = Context.Rel.Declaration.LocalAssum (n, ty) in
    let loc = LocStart { elims; context; head=t; stack=stk; next=Continue next } in
...
| FFlex _ -> v.mark == Ntrl
| FConstruct _ | FApp _ | FProj _ | FFix _ | FCoFix _ | FCaseT _ | FCaseInvert _ | FLambda _ | FLambdaF _
