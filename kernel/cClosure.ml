...
module ConvCache = struct
  ...
  (* Multiplicative hash constants. On 63-bit ints the multipliers are
     0x9E3779B97F4A7C1 and 0x3F58476D1CE4E5B9, assembled from pieces because
     such literals do not parse on 32-bit OCaml, which gets narrower
     multipliers and shifts instead. *)
  let mult1, mult2, shift1, shift2 =
    if Sys.int_size >= 63 then
      (0x9E3779B lsl 32) lor (0x97F lsl 20) lor 0x4A7C1,
      (0x3F58476 lsl 36) lor (0xD1CE lsl 20) lor 0x4E5B9,
      29, 32
    else 0x9E3779B, 0x3F58476D, 15, 16

  let mix pk m =
    let h = pk + m * mult1 in
    let h = h lxor (h lsr shift1) in
    let h = h * mult2 in
    h lxor (h lsr shift2)
