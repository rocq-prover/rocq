...
let it_mkLambda_or_LetIn infos ctx t =
  ...
  | LocalAssum (n, ty) :: ctx ->
      ...
      { ctx=None; term = FLambda(List.length assums, assums, Term.it_mkLambda_or_LetIn (term_of_fconstr t) (List.rev ctx), (subs_id l, UVars.Instance.empty)); mark = t.mark }
  | LocalDef _ :: _ ->
      mk_clos None (subs_id l, UVars.Instance.empty) (Term.it_mkLambda_or_LetIn (term_of_fconstr t) ctx)
  ...
  | FConstruct (c,_) ->
     ...
     if use_match || use_fix then
      (match [@ocaml.warning "-4"] m,  stk with
        ...
        | (c, ZcaseT(case_ctx, ci,_,pms,_,br,e)::s) when use_match ->
            assert (ci.ci_npar>=0);
        ...
        | (m, Zproj (proj_ctx,p,_)::s) when use_match ->
