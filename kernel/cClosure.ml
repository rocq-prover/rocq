...
and comp_subs ~(info:clos_infos) ~(tab:clos_tab) el (s,u') =
  Esubst.lift_subst (fun el c -> lazy (Vars.make_substituend @@ to_constr ~info ~tab el c)) el s, u'
...
let subst_context info tab env ctx =
  if Esubst.is_subs_id (fst env) then
    subst_instance_context (snd env) ctx
  else
    let subs = comp_subs ~info ~tab el_id env in
    let subst_constr = subst_constr ~mode:normal_whnf info tab in
...
let it_mkLambda_or_LetIn info tab ctx t =
  let l = Range.length (info_relevances info) in
  let open Context.Rel.Declaration in
  ...
  | LocalAssum (n, ty) :: ctx ->
      ...
      { term = FLambda(List.length assums, assums, Term.it_mkLambda_or_LetIn (term_of_fconstr ~info ~tab t) (List.rev ctx), (Esubst.subs_id l, UVars.Instance.empty)); mark = t.mark }
  | LocalDef _ :: _ ->
      mk_clos ~mode:normal_whnf (Esubst.subs_id l, UVars.Instance.empty) (Term.it_mkLambda_or_LetIn (term_of_fconstr ~info ~tab t) ctx)
...
let inductive_subst ~mode mib u pms =
  ...
  | RelDecl.LocalAssum _ :: ctx ->
    ...
    Esubst.subs_cons (pms.(i)) subs
  | RelDecl.LocalDef (_, c, _) :: ctx ->
    let subs = mk_pms i ctx in
    Esubst.subs_cons (mk_clos ~mode (subs,u) c) subs
  in
...
and match_elim : 'a. ('a, 'a depth) reduction -> _ -> _ -> pat_state:'a depth -> _ -> _ -> _ -> _ -> _ -> _ -> 'a =
  ...
  | ZcaseT (ci, u, pms, (p, r), brs, e, _mode) :: s ->
      ...
      let ntys_ret = subst_context info tab e ntys_ret in
      let ret = mk_clos ~mode:normal_whnf (usubs_liftn (Context.Rel.length ntys_ret) e) (snd p) in
      let brs = Array.map2 (fun ctx br -> subst_context info tab e ctx, mk_clos ~mode:normal_whnf (usubs_liftn (Context.Rel.length ctx) e) (snd br)) ntys_brs brs in
...
and match_arg : 'a. ('a, 'a depth) reduction -> _ -> _ -> pat_state:'a depth -> _ -> _ -> _ -> _ -> _ -> 'a =
  ...
  let t' = it_mkLambda_or_LetIn info tab context t in
...
and match_head red info tab ~pat_state next context states patterns t stk =
  ...
  | FProd (n, ty, body, e) ->
    ...
    let ctx1 = List.map (fun (n, ty) -> Context.Rel.Declaration.LocalAssum (n, ty)) ntys |> subst_context info tab e in
    let ctx = ctx1 @ [Context.Rel.Declaration.LocalAssum (n, term_of_fconstr ~info ~tab ty)] in
    let ntys'' = List.mapi (fun n (_, t) -> mk_clos ~mode:normal_whnf (usubs_liftn n e) t) (List.rev ntys) in
    let tys = Array.of_list (ty :: ntys'') in
    let contexts_upto = Array.init na (fun i -> List.lastn i ctx @ context) in
    let loc = LocStart { elims; context; head=t; stack=stk; next=Continue next } in
    let loc = LocArg { patterns = pbody; context = ctx @ context; arg = mk_clos ~mode:normal_whnf (usubs_liftn na e) body; next = loc } in
  ...
  | FLambda (na, ntys, body, e) ->
    ...
    let ctx = List.rev_map (fun (n, ty) -> Context.Rel.Declaration.LocalAssum (n, ty)) ntys |> subst_context info tab e in
    let tys = Array.of_list ntys in
    let tys = Array.mapi (fun n (_, t) -> mk_clos ~mode:normal_whnf (usubs_liftn n e) t) tys in
    let contexts_upto = Array.init na (fun i -> List.lastn i ctx @ context) in
    let loc = LocStart { elims; context; head=t; stack=stk; next=Continue next } in
    let loc = LocArg { patterns = pbody; context = ctx @ context; arg = mk_clos ~mode:normal_whnf (usubs_liftn na e) body; next = loc } in
    let loc = Array.fold_right3 (fun patterns arg context next -> LocArg { patterns; context; arg; next }) (Array.transpose (Array.map (Status.split_array na) ptys)) tys contexts_upto loc in
...
let rec knr info tab ~pat_state m stk =
  ...
  | FCaseInvert (ci, u, pms, _p,iv,_c,v,env) when red_set mode info RedFlags.fMATCH ->
    ...
    | Some c -> knit ~mode:mode info tab ~pat_state env c stk
