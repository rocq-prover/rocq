...
let find_rectype_codom ?evars env ty =
  let ctx, ret = whd_decompose_prod_decls ?evars env ty in
...
let lazy_apply_filter_spec not_subterm ?evars spec = function
  | Pass -> spec
  | Block -> not_subterm
  | Prune (env, ind, args) -> lazy (
      Subterm.prune_path ?evars env (Lazy.force spec) ind args
    )
...
let filter_type ?evars pctxlen env k t =
  ...
  if noccur_with_meta 1 pctxlen p then
    ...
    let rec filter_stack env k p res = function
      ...
      | elt :: stack ->
        ...
        | Prod (na, ty, p) ->
          let filter = filter_type ?evars pctxlen env k ty in
          let spec = stack_element_specif ?evars elt in
          let elt = SArg (lazy_apply_filter_spec not_subterm spec filter) in
...
let filter_predicate_codomain ?evars pctxlen ret spec =
  ...
  | Test (env, k, ty) ->
    let filter = filter_type ?evars pctxlen env k ty in
    apply_filter_spec Subterm.not_subterm spec filter

let filter_predicate_tree ?evars pctxlen env p elt =
  if noccur_with_meta 1 pctxlen p then
    Some elt
  else
    let filter = filter_type ?evars pctxlen env 0 p in
    apply_filter_tree elt filter
  ...
  let rec check_rec_call_stack renv stack rs t =
    ...
    | Case (ci, u, pms, (p, _), _, c, brs) -> (* iv ignored: it's just a cache *)
      ...
      let rs =
        ...
        check_inert_subterm_rec_call renv rs (snd p)
      in
      let rs' = NoNeedReduce :: rs in
      let nr = redex_level rs' in

      let filtered_stack, _ =
        let not_subterm = Lazy.from_val (Subterm.internal nr) in
        filter_predicate_stack stack_element_specif ?evars not_subterm pctxlen penv (snd p) stack
      in
    ...
    if not (noccur_with_meta n nbfix t) then
      ...
      match kind c with
        ...
        | Case (ci, u, pms, (p, _), _, tm, brs) -> (* iv ignored: just a cache *)
          ...
          in
          let () = if not (noccur_with_meta (n + pctxlen) nbfix (snd p)) then
