...
let get_cumulativity_constraints cv_pb ~nargs variance u u' =
  let pri = UVars.Instance.pr Sorts.QVar.raw_pr Univ.Level.raw_pr in
  ...
  match cv_pb with
  | CONV ->
    UVars.enforce_eq_variance_instances ~nargs variance u u' (UVars.QPairSet.empty, Univ.UnivConstraints.empty)
  | CUMUL ->
    UVars.enforce_leq_variance_instances ~nargs variance u u' (UVars.QPairSet.empty, Univ.UnivConstraints.empty)
