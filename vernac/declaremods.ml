...
(** {6 Inlining and functor application}

    A module's delta-resolver tells which of its fields are aliases of which
    other names. The substitution generated by applying a functor tells which
    constants are to be replaced by the body the argument gives them. The object
    substitution of a module defined by a functor application needs both, which
    we compute separately. *)

let rec decompose_application accu = function
  | MEident mp -> Some (mp, accu)
  | MEapply (f, mp) -> decompose_application (mp :: accu) f
  | MEwith _ -> None

(** The bodies the arguments of [me] give to the fields their parameter type
    declared inlinable, indexed by both the user and the canonical name of the
    argument's field. *)
let inlined_bodies env inl ~is_mod me =
  match decompose_application [] me with
  | None | Some (_, []) -> KerName.Map.empty
  | Some (mp1, mp_l) ->
    let rec collect sign mp_l accu = match mp_l with
    | [] -> accu
    | mparg :: mp_l ->
      let farg_id, farg_b, fbody_b = Modops.destr_functor sign in
      let accu = match mod_global_delta (Environ.lookup_module mparg env) with
  ...
  if KerName.Map.is_empty inlined then accu
    ...
    let rec walk mp struc accu =
      ...
      | SFBmodule mb ->
        ...
        | NoFunctor struc -> walk (MPdot (mp, l)) struc accu
        | MoreFunctor _ -> accu
        end
      | SFBmind _ | SFBrules _ | SFBmodtype _ -> accu
      in
...
let declare_module id args res mexpr_o =
  ...
  in
  (* Computed here, while the parameters of the module are still part of the
     environment. *)
  let inlined = match entry with
    | MType (_,mte) | MExpr (_,_,Some mte) ->
      inlined_bodies env inl_res ~is_mod:false mte
    | MExpr (_,me,None) ->
      inlined_bodies env inl_expr ~is_mod:true me
  in
  (* Undo the simulated interactive building of the module
     and declare the module as a whole *)
  ...
  let sobjs =
    subst_sobjs
      (map_mp mp0 mp (subst_reso_of_module (Global.env ()) inlined mp resolver))
      sobjs
  in
...
let declare_one_include_core (me,base,kind,inl) =
  ...
  let struc = match RawModOps.Interp.current_struct () with
  | NoFunctor struc -> struc
  | MoreFunctor _ -> assert false
  in
  let subst =
    join subst_self
      (map_mp base_mp cur_mp
        (RawModOps.Interp.subst_reso_of_struct inlined cur_mp struc resolver))
  in
