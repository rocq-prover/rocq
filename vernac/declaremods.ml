...
let check_sub env mp sub_mtb_l =
  let fold sub_mtb (cst, env) =
    ...
    let ugraph, qgraph, cst = Subtyping.check_subtypes state env mp mp sub_mtb in
...
let build_subtypes env mp args mtys =
  let (ctx, ans) = List.fold_left_map
    (fun ctx (mte,base,kind,inl) ->
       ...
       let state = ((Environ.universes env, Environ.qualities env, Univ.Constraints.empty), Reductionops.inferred_universes) in
...
let intern_arg (acc, cst) (mbidl,(mty, base, kind, inl)) =
  ...
  let () =
    let state = ((Global.universes (), Global.elim_graph (), Univ.Constraints.empty), Reductionops.inferred_universes) in
...
let declare_one_include_core (me,base,kind,inl) =
  ...
  let rec compute_sign sign =
    ...
    | MoreFunctor(mbid,mtb,str) ->
      ...
      let (_, _, cst) = Subtyping.check_subtypes state env cur_mp (MPbound mbid) mtb in
      let () = Global.add_constraints cst in
