...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let collapse_sort_variables = not (List.is_empty udecl.univdecl_qualities && (not @@ PolyFlags.collapse_sort_variables poly)) in
  let poly = PolyFlags.set_collapse_sort_variables poly collapse_sort_variables in
