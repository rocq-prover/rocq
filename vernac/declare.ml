...
type default_body_opacity =
  ...
  | Opaque of PConstraints.ContextSet.t
...
let make_univs_immediate_default ~poly ~opaque ~uctx ~udecl ~eff ~used_univs body typ =
  ...
  let utyp = match fst utyp with
    ...
    | Monomorphic_entry uctx ->
      ...
      Monomorphic_entry (PConstraints.ContextSet.union uctx (Safe_typing.universes_of_private eff.Evd.seff_private)), snd utyp
  in
  uctx, utyp, used_univs, Default { body = (body, eff); opaque = if opaque then Opaque PConstraints.ContextSet.empty else Transparent }
...
let definition_entry ?(opaque=false) ?using ?inline ?types ?univs body =
  let opaque = if opaque then Opaque PConstraints.ContextSet.empty else Transparent in
...
module ProofEntry = struct
  ...
  let force_extract_body entry =
    match entry.proof_entry_body with
    | Default { body = (body, eff); opaque = Transparent } -> ((body, PConstraints.ContextSet.empty), eff), false, None
  ...
  let get_entry_body entry =
    ...
    let uctx = match opaque with
      | Opaque uctx -> uctx
      | Transparent -> PConstraints.ContextSet.empty
    in
    (body, uctx), eff
  ...
  let impl,opaque = match d with (* Fails if not well-typed *)
    ...
    | SectionLocalDef { clearbody; entry = de } ->
      ...
      let se = if opaque then
          ...
          let de = {
            proof_entry_body = DeferredOpaque { body = Future.from_val ((body, PConstraints.ContextSet.empty), Evd.empty_side_effects); feedback_id };
...
let prepare_proof ?(warn_incomplete=true) { proof; pinfo } =
  ...
  let proofs = match pinfo.possible_guard with
    ...
    | Some (possible_guard, fixrelevances) ->
      ...
      fst (make_recursive_bodies ~elim_to:(Inductive.eliminates_to (Evd.elim_graph evd))
             env ~typing_flags ~possible_guard ~rec_declaration) in
