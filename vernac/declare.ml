...
let declare_constant sum ~loc ?(local = Locality.ImportDefaultBehavior)
    ~name ~kind ~typing_flags ?user_warns cd =
    ...
    | SectionLocalAssum {typ;impl;univs} ->
      let () = match fst univs with
        | UState.Monomorphic_entry uctx ->
          ...
          DeclareUniv.name_mono_section_univs sum (fst uctx);
...
let declare_mutual_definitions sum ~info ~cinfo ~opaque ~uctx ~bodies ~possible_guard ?using () =
  ...
  let pm =
    if pred rem > 0 then
      ...
      if not (Int.Set.is_empty deps) then
        let pm, _progress = auto sum ~pm (Some prg.prg_cinfo.CInfo.name) deps None in
        pm
      else pm
    else pm
  in
...
let obligation_terminator sum ~pm ~entry ~uctx ~oinfo:{name; num; auto; check_final} =
  let env = Global.env () in
...
let finish_proved_equations sum ~pm ~kind ~hook i entries types sigma0 =
  ...
  let sigma, recobls =
    CList.fold_left2_map (fun sigma (_evar_env, ev, _evi, local_context, _type) entry ->
        ...
        let cst = declare_constant sum ~loc:None ~name:id ~kind ~typing_flags:None
...
let finish_proof sum ~pm proof_obj proof_info =
  ...
  | End_obligation oinfo ->
    ...
    Obls_.obligation_terminator sum ~pm ~entry ~uctx ~oinfo
...
let solve_obligation ?check_final prg num tac =
  ...
  let auto sum ~pm n oblset tac = auto_solve_obligations sum ~pm n ~oblset tac in
  let proof_ending =
...
let solve_obligations sum ~pm n tac =
  let prg = get_unique_prog ~pm n in
  solve_prg_obligations sum ~pm prg tac
...
let try_solve_obligations sum ~pm name tac =
  solve_obligations sum ~pm name tac |> fst
...
let add_definition sum ~pm ~info ~cinfo ~opaque ~uctx ?body
  ...
  if Int.equal (Array.length obls) 0 then (
    ...
    let pm, cst = Obls_.declare_definition sum ~pm prg in
    pm, Defined cst)
  else
