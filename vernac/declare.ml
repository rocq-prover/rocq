...
| UState.Monomorphic_entry uctx, unames ->
  ...
  let usubst, uctx = UnivGen.fresh_universe_context_set_instance uctx in
  let uctx = PConstraints.ContextSet.univ_context_set uctx in
  let usubst = (Sorts.QVar.Map.empty, usubst) in
  ...
  let guctx = PConstraints.ContextSet.of_univ_context_set guctx in
  guctx, (UState.Monomorphic_entry uctx, unames), usubst
| UState.Polymorphic_entry _, _ ->
  PConstraints.ContextSet.empty, univs, UVars.empty_sort_subst
...
let build_constant_by_tactic ~name ?warn_incomplete ~sigma ~env ~sign ~poly (typ : EConstr.t) tac =
  ...
  let sigma = Evd.merge_context_set Evd.univ_rigid sigma uctx in
  entry, status, sigma
