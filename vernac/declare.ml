...
let declare_definition ~info ~cinfo ~opaque ~obls ~body ?using sigma =
  ...
  let sigma = Evd.minimize_universes ~collapse_sort_variables:(PolyFlags.collapse_sort_variables info.Info.poly) sigma in
...
let prepare_obligations ~name poly ?types ~body env sigma =
  ...
  let sigma, (body, types) = Evarutil.finalize ~collapse_sort_variables:(PolyFlags.collapse_sort_variables poly)
      ~abort_on_undefined_evars:false
...
let prepare_parameter ~poly ~udecl ~types sigma =
  ...
  let sigma, typ = Evarutil.finalize ~collapse_sort_variables:(PolyFlags.collapse_sort_variables poly)
                   ~abort_on_undefined_evars:true
...
let prepare_proof ?(warn_incomplete=true) { proof; pinfo; sideff } =
  ...
  let evd = Evd.minimize_universes ~collapse_sort_variables:(PolyFlags.collapse_sort_variables poly) evd in
