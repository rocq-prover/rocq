...
let finalize_def_class ~poly env sigma ~params ~sort ~projtyp =
  let sigma, (params, sort, typ, projtyp) =
    Evarutil.finalize ~abort_on_undefined_evars:false
      ~collapse_sort_variables:(PolyFlags.collapse_sort_variables poly) ~partial:false
      sigma (fun nf ->
...
let typecheck_params_and_fields ~kind ~(flags:ComInductive.flags) ~primitive_proj udecl params (records : DataI.t list) =
  ...
  let sigma, (impls_env, ((_env1,params), impls, _paramlocs)) =
    Constrintern.interp_context_evars ~program_mode:false ~poly:flags.poly ~unconstrained_sorts env0 sigma params in
  let sigma, typs =
