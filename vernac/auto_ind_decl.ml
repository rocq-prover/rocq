...
let get_inductive_deps ~noprop env kn =
  ...
  let get_deps_one accu i mip =
    ...
    let rec aux env accu c =
      ...
      | Float _ | String _ | Array _ | PBlock _ | PUnblock _ | PRun _ -> Termops.fold_constr_with_full_binders env sigma EConstr.push_rel aux env (List.fold_left (aux env) accu a) c
    in
...
let build_beq_scheme env handle kn =
  ...
  *)
  ...
  let rec translate_type_eq env_lift na c t =
    ...
    (* Not building a type *)
    | Proj _ | CoFix _ | Int _ | Float _ | String _ | PBlock _ | PUnblock _ | PRun _ -> None
  ...
  and translate_term_eq env_lift c =
    ...
    | Int _ | Float _ | String _ | Array _ | PBlock _ | PUnblock _ | PRun _ -> None
    in
  ...
  let truly_recursive =
    ...
    Array.exists Inductiveops.mis_is_recursive mib.mind_packets
  in
