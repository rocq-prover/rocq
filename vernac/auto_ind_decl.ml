...
let _dbg = CDebug.create ~name:"auto_ind_decl" ()
...
(*           let uctx = Declareops.constant_polymorphic_context (Environ.lookup_constant c env) in
          let _ = dbg (fun () -> Printer.pr_abstract_universe_ctx sigma uctx) in
          let _ = dbg (fun () -> Printer.pr_universe_instance sigma u) in
          let sigma, _ = Typing.type_of ~refresh:true env sigma (EConstr.of_constr blI) in
