...
let do_mutually_recursive sum ?pm ~refine ~program_mode ?(use_inference_hook=false) ?scope ?clearbody ~kind ~poly ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  | Some pm ->
    ...
    | [Some _], [body], [cinfo], [obls] ->
      ...
      let pm, _ = Declare.Obls.add_definition sum ~pm ~cinfo ~info ~opaque:false ~body ~uctx ?using obls in
      Some pm, None
    | _ ->
  ...
  | None ->
    ...
    | Some bodies ->
      ...
      let _ : GlobRef.t list =
        Declare.declare_mutual_definitions sum ~cinfo ~info ~opaque:false ~uctx ~possible_guard ~bodies ?using ()
      in
      None, None
    | None ->
