...
let template_poly_variables env ind =
  ...
  | Some { template_defaults; template_concl } ->
    let pseudo_poly = match template_concl with
      | VQSort (q, _) when Option.has_some (Sorts.QVar.var_index q) -> true
      | _ -> false
    in
  ...
  | GlobRef.IndRef ind ->
    ...
    | Some squash ->
      ...
      let target = match inds with
          ...
          | GQSort (q,_) -> str "in sort quality " ++ Termops.pr_evd_qglobal sigma q
          | VQSort (q,_) -> str "in sort quality " ++ Termops.pr_evd_qvar sigma q
      in
