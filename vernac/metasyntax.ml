...
(* Worth reporting because the declared levels still reach printing: through the
   unparsing rule when the declaration carries a format, and always through the
   subentry levels of its interpretation. *)
...
let cache_one_syntax_extension (ntn,synext) =
  ...
  in
  (* True for an "only printing" declaration, and on [Import] for a leaf whose
     level [recover_notation_syntax] read back from such a string. *)
    ...
    try
      ...
      if not (level_eq prec oldprec && List.for_all2 Extend.constr_entry_key_eq synext.synext_nottyps oldtyps) then begin
        if no_parsing_rule || Option.is_empty oldparsing then
          (* A notation string has at most one grammar rule, so there is a
             level to agree on only when both sides record one.  See #12465,
             #12589 and #6078. *)
