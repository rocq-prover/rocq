...
let do_symbol ~poly ~unfold_fix udecl (id, typ) =
  ...
  let univs = Evd.check_univ_decl ~poly ~kind:PolyFlags.Assumption evd udecl in
...
let update_invtblu ~loc evd (qsubst, usubst) (state, stateq, stateu : state) u : state * _ =
  ...
  let stateu, masku = Array.fold_left_map (fun stateu lvlold ->
   let levopt = Univ.Universe.level (UVars.subst_univs_level_universe usubst lvlold) in
   match levopt with
   | Some lev -> let lvlnew = Univ.Level.var_index lev in
                 Option.fold_right (update_invtblu1 ~loc evd lvlold) lvlnew stateu, lvlnew
   | None -> CErrors.user_err ?loc Pp.(str"Only level variables may appear in a universe pattern"))
   stateu u
  in
...
let interp_rule (udecl, lhs, rhs: Constrexpr.universe_decl_expr option * _ * _) =
  ...
  let evd, udecl =
    ...
    let decl = {
      ...
      univdecl_variances = None; (* FIXME: MS *)
