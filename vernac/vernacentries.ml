...
(* Fold a list of declaration hooks into a single hook that runs them all,
   in order, when the constant is saved. *)
let hook_of_hooks hooks =
  match hooks with
  | [] -> None
...
let vernac_start_proof ~atts kind l =
  ...
  let hook = hook_of_hooks hooks in
